...
namespace App\Standards\RouteGroups\Abstracts;
...
abstract class RouteGroup
{
     ...
     * @return bool
     */
    static public function has_sequence(string $class, string $pattern = null): bool
    {
        if (empty($pattern))
        {
            return self::$sequences->has($class);
        }

        $pattern = preg_replace([ '/\./', '/\*/' ], [ '\.', '.*' ], $pattern);

        return self::$sequences->has($class) && preg_match('/' . $pattern . '/', self::get_sequence($class));
    }
